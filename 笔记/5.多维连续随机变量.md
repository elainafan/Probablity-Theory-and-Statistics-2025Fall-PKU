...
$$
\operatorname{Var}\left(\sum_i X_i\right)=\sum_i\sum_j\operatorname{Cov}(X_i,X_j).
$$
...
并且 $\operatorname{Corr}(X,Y)=\pm 1$ 当且仅当 $Y=aX+b$ 几乎必然成立，其中 $a\neq 0$ 。
...
若 $X,Y$ 独立且均服从 $\operatorname{Exp}(1)$ ，则
...
$$
\operatorname{Cov}(X)=E((X-E(X))(X-E(X))^\top).
$$
