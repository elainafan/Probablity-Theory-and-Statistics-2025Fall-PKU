...
对 $n_A\sim B(n,p)$ ，有 $\operatorname{Var}(n_A)=np(1-p)$ ，因此
...
Chernoff bound 的做法是对 $e^{tX}$ 使用 Markov 不等式。若 $t>0$ ，则
...
$$
\operatorname{Var}\left(\sum_{i=1}^{n}X_i\right)=O(n),
$$
...
Berry-Esseen 定理给出收敛速度。若 $E(|X_i-\mu|^3)<+\infty$ ，则对任意 $x$ 有
...
令 $C_{ij}$ 表示排序过程中元素 $i$ 与 $j$ 是否被比较。若 $i<j$ ，则它们被比较当且仅当区间 $\{i,i+1,\ldots,j\}$ 中第一个被选为主元的元素是 $i$ 或 $j$ 。因此
...
$$
E(T)=O\left(\sum_{i<j}\frac{2}{j-i+1}\right)=O(n\log n).
$$
...
给定集合族 $S_1,\ldots,S_m\subseteq \{1,\ldots,n\}$ ，随机把每个元素染成 $+1$ 或 $-1$ 。对固定集合 $S_i$ ，定义

$$
\operatorname{disc}_\chi(S_i)=\left|\sum_{j\in S_i}\chi(j)\right|.
$$

由 Chernoff-Hoeffding 不等式

$$
P(\operatorname{disc}_\chi(S_i)\geq k)\leq 2e^{-k^2/(2n)}.
$$
