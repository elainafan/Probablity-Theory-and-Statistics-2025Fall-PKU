...
若 $\operatorname{Bias}(\hat{\theta})=0$ ，则称估计量无偏。若
...
$$
\operatorname{MSE}(\hat{\theta}_n)\to 0.
$$
...
\leq
\frac{\operatorname{MSE}(\hat{\theta}_n)}{\epsilon^2}.
$$
...
$$
\operatorname{MSE}(\bar{X})=\operatorname{Var}(\bar{X})=\frac{\operatorname{Var}(X)}{n}.
$$
...
不是 $\operatorname{Var}(X)$ 的无偏估计，因为

$$
E(B_2)=\frac{n-1}{n}\operatorname{Var}(X).
$$
...
例如若总体 $X\sim \operatorname{Exp}(\lambda)$ ，则
...
$$
\operatorname{softmax}(a)_i=\frac{e^{a_i}}{\sum_j e^{a_j}}.
$$
...
$$
P(|\bar{X}-p|>\epsilon)\leq 2e^{-2n\epsilon^2}.
$$
